...
function youtube_embed_to_short_code( $content ) {
	...
	foreach ( array( 'regexp', 'regexp_ent', 'old_regexp', 'old_regexp_ent', 'ifr_regexp', 'ifr_regexp_ent' ) as $reg ) {
		...
		foreach ( $matches as $match ) {
			 ...
			 */
			do_action( 'kentblogs_embed_to_shortcode', 'youtube', $url );
		}
	}
...
function youtube_id( $url ) {
	if ( ! $id = kentblogs_get_youtube_id( $url ) )
	 ...
	 */
	if ( apply_filters( 'kentblogs_youtube_allow_autoplay', false ) && isset( $qargs['autoplay'] ) )
 ...
 * http://www.youtube.com/v/9FhMMmqzbD8?fs=1&hl=en_US
 */
function kentblogs_youtube_embed_crazy_url( $matches, $attr, $url ) {
	return youtube_id( $url );
}

function kentblogs_youtube_embed_crazy_url_init() {
	wp_embed_register_handler( 'kentblogs_youtube_embed_crazy_url', '#https?://(?:www\.)?(?:youtube.com/(?:v/|playlist|watch[/\#?])|youtu\.be/).*#i', 'kentblogs_youtube_embed_crazy_url' );
}

add_action( 'init', 'kentblogs_youtube_embed_crazy_url_init' );
...
*/
function kentblogs_get_youtube_id( $url ) {
